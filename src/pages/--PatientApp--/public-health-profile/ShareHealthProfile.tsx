...
const ShareHealthProfile = () => {
  ...
  const {
    ...
    `${apiRoutes.getPublicProfile}/${token}`,
    ["getPublicProfile"],
    !!token
  );

  const typedFetchError = fetchError as ApiError | undefined;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <div className="h-10 w-10 border-4 border-[#009FB6] border-t-transparent rounded-full animate-spin" />
          <p className="text-sm text-[#424242] font-mona">
            Loading public health profile...
          </p>
        </div>
      </div>
    );
  ...
  if (typedFetchError) {
    const isNotFoundError =
      typedFetchError?.status === 404 ||
      typedFetchError?.response?.data?.message ===
        "Public profile not found or disabled";
        ...
        <div className="min-h-screen flex items-center justify-center px-4">
          <div className="text-center space-y-6 max-w-md">
            <div className="w-20 h-20 flex items-center justify-center mx-auto">
              <img src="/../icons/!-sign-color-icon.svg" alt="icon" />
            </div>
            ...
            <p className="text-gray-600">
              This profile is not available. It may have been disabled or the
              link is invalid.
            </p>

            <button
              onClick={() => (window.location.href = "/")}
              className="px-6 py-3 bg-[#009FB6] text-white rounded-xl font-medium hover:bg-[#0088A3] transition-colors cursor-pointer"
            >
              Return to Home
            </button>
          </div>
...
export default ShareHealthProfile;
