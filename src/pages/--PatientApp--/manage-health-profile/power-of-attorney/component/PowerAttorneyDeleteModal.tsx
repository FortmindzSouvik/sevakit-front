...
export default function PowerAttorneyDeleteModal({
  isOpen,
  onClose,
  onSuccess,
  powerOfAttorneyId,
  ...
  const { mutateAsync: deletePowerOfAttorney, isPending } = useDeleteData([
    "delete-power-of-attorney",
  ]);
  const handleDelete = async () => {
    try {
      await deletePowerOfAttorney(
        `${apiRoutes.updatePowerOfAttorney(powerOfAttorneyId)}`
      );
    } catch (error) {
      ...
      <DialogContent className="max-w-md p-9 rounded-2xl">
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <p className="text-[#000000] text-base leading-relaxed font-mona">
            This will remove the Power of Attorney’s information and all
            attached legal documents from your profile.
          </p>
