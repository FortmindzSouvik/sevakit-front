...
export default function InsuranceDeleteModal({
  isOpen,
  onClose,
  onSuccess,
  insuranceId,
  ...
  const { mutateAsync: deleteInsurance, isPending } = useDeleteData([
    "delete-insurance",
  ]);
  const handleDelete = async () => {
    try {
      await deleteInsurance(`${apiRoutes.updateInsurance(insuranceId)}`);
      toast.success("Insurance information deleted successfully.");
    } catch (error) {
      ...
      <DialogContent className="max-w-md p-9 rounded-2xl">
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <p className="text-[#000000] text-base leading-relaxed font-mona">
            This insurance will be removed from your health profile.
          </p>
