...
export default function CaregiverDeleteModal({
  isOpen,
  onClose,
  onSuccess,
  careGiverId,
  ...
  const { mutateAsync: deleteCareGiver, isPending } = useDeleteData([
    "delete-caregiver",
  ]);
  const handleDelete = async () => {
    try {
      await deleteCareGiver(`${apiRoutes.updateCareGiver(careGiverId)}`);
    } catch (error) {
      ...
      <DialogContent className="max-w-md p-9 rounded-2xl">
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <p className="text-[#000000] text-base leading-relaxed font-mona">
            This will remove your Caregiver’s details from your health profile.
          </p>
