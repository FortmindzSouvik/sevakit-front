...
export default function DriversLicenseDeleteModal({
  isOpen,
  onClose,
  onSuccess,
  drivingLicenseId,
  ...
  const { mutateAsync: deleteDrivindLicenseSymptoms, isPending } =
    useDeleteData(["delete-dl"]);
  const handleDelete = async () => {
    try {
      await deleteDrivindLicenseSymptoms(
        `${apiRoutes.updateDrivingLicense(drivingLicenseId)}`
      );
      ...
      <DialogContent className="max-w-md p-9 rounded-2xl">
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <p className="text-[#000000] text-base leading-relaxed font-mona">
            This will remove your Driver’s License details from your health
            profile.
          </p>
          ...
          <DialogFooter className="w-fit flex items-center justify-center gap-6 pt-2 ">
            ...
            <button
              className="flex justify-center items-center w-fit h-fit px-10 py-4 bg-[#FF5A54] text-[#212121] rounded-xl 
