...
export default function PrimaryCareDeleteModal({
  isOpen,
  onClose,
  onSuccess,
  ...
  const { mutateAsync: deletePharmacy, isPending } = useDeleteData([
    "delete-pharmacy",
  ]);

  const handleDelete = async () => {
    try {
      await deletePharmacy(`${apiRoutes.updatePrimaryCareProvider(primaryCareProviderId)}`);
      toast.success("Care provider details deleted successfully.");
    } catch (error) {
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-black font-mona">
              Delete Care Providers?
            </DialogTitle>
          </DialogHeader>

          <p className="text-[#000000] text-base leading-relaxed font-mona">
            This will remove your provider’s details from your health profile.
          </p>
