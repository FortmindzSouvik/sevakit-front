...
export default function AdvancePlanDeleteModal({
  isOpen,
  onClose,
  onSuccess,
  advancePlanId,
  ...
  const { mutateAsync: deleteSymptoms, isPending } = useDeleteData([
    "delete-advance-care-plan",
  ]);
  const handleDelete = async () => {
    try {
      await deleteSymptoms(`${apiRoutes.updateAdvanceCarePaln(advancePlanId)}`);
      toast.success("Advance care plan deleted successfully.");
    } catch (error) {
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-black font-mona">
              Delete Advanced Care Plan?
            </DialogTitle>
          </DialogHeader>

          <p className="text-[#000000] text-base leading-relaxed font-mona">
            Your care preferences will be removed from your profile.{" "}
          </p>
